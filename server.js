// 🚀 Ghost Store BD & SPARK AI BD - Gemini 3.6 Flash Active Bot (server.js)
...
// ⚡ HTTP Keep-Alive Agent (কানেকশন স্পিড বাড়ানোর জন্য)
const axiosFast = axios.create({
    httpAgent: new http.Agent({ keepAlive: true }),
    httpsAgent: new https.Agent({ keepAlive: true }),
    timeout: 5000 // ৫ সেকেন্ডের বেশি টাইম লাগলে ব্যাকআপ মডেলে যাবে
});
...
// ⚡ ইন-মেমোরি ক্যাশিং (স্পিড ফাস্ট রাখার জন্য)
let cachedSettings = null;
let lastCacheTime = 0;

async function getBotSettingsCached() {
    const now = Date.now();
    if (cachedSettings && (now - lastCacheTime < 60000)) { // ১ মিনিটের ইন-মেমোরি ক্যাশ
...
// 👤 ফেসবুক কাস্টমারের আসল নাম সংগ্রাহক
        ...
        for (let entry of body.entry) {
            if (entry.messaging && entry.messaging[0]) {
                ...
                if (sender_psid && webhook_event.message && webhook_event.message.text) {
                    ...
                    // টাইপিং ইন্ডিকেটর অন করা (Non-blocking)
...
async function processUserMessageFast(sender_psid, userMsg) {
    ...
    // ৩. ব্যাকগ্রাউন্ডে ফায়ারবেস লগ সেভ (Non-blocking)
    ...
    // ৪. Gemini 3.6 Flash রিকোয়েস্ট প্রসেস
    ...
    let customModel = settings.aiModel || "gemini-3.6-flash";
...
// 🤖 Gemini 3.6 Flash আল্ট্রা-ফাস্ট ফাংশন
...
${customAdminPrompt ? `[এডমিন বিশেষ নির্দেশিকা]:\n${customAdminPrompt}` : ''}`;
    ...
    // 🎯 Gemini 3.6 Flash-কে এক নম্বর অগ্রাধিকার দেওয়া হয়েছে
    const models = [
    ...
    const payload = {
        ...
        generationConfig: {
            maxOutputTokens: 600, // ফাস্ট ও গুছানো উত্তরের জন্য
            temperature: 0.7
        }
    };
...
app.listen(PORT, () => console.log(`SPARK Gemini 3.6 Flash Active Bot running on port ${PORT}`));
