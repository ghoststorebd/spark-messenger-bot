// 🚀 Ghost Store BD & SPARK AI BD - Facebook Real Profile Name Sync Bot (server.js)
...
async function getFacebookUserProfile(psid, accessToken) {
    try {
        const res = await axios.get(`https://graph.facebook.com/v19.0/${psid}?fields=first_name,last_name,profile_pic&access_token=${encodeURIComponent(accessToken)}`);
        ...
        for (let entry of body.entry) {
            if (entry.messaging && entry.messaging[0]) {
                ...
                if (sender_psid && webhook_event.message && webhook_event.message.text) {
                    ...
                    // 👤 ফেসবুক থেকে কাস্টমারের আসল নাম সংগ্রহ করা
                    ...
                    if (userModeDoc.exists) {
                        ...
                        // নতুন ইউজারকে ফায়ারবেসে আসল নামসহ সেভ করা এবং ওয়েলকাম মেসেজ পাঠানো
...
async function getGeminiReply(userMsg, apiKeyFromDb, botName, customAdminPrompt, customModel) {
    let systemInstructionText = `তুমি "Ghost Store BD" এর সাপোর্ট বট ${botName}। 
...
app.listen(PORT, () => console.log(`SPARK Real FB Name Sync Bot running on port ${PORT}`));
